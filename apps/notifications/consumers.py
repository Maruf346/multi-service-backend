...
class NotificationConsumer(AsyncWebsocketConsumer):
    """SuperAdmin-only websocket consumer for live notification delivery."""

    async def connect(self):
        token = self._extract_token()
        self.user = await self.get_user_from_token(token)

        if not await self.is_allowed_superadmin(self.user):
            await self.close(code=4003)
            return

        self.group_name = f'superadmin_notifications_{self.user.id}'
        await self.channel_layer.group_add(self.group_name, self.channel_name)
        await self.accept()
        await self.send(text_data=json.dumps({
            'type': 'connection_established',
            'message': 'Connected to SuperAdmin notification service.',
    ...
    @database_sync_to_async
    def is_allowed_superadmin(self, user):
        return bool(
            user
